...
function [acc nmi purity] = myProposedMethodRevised(X,Z_ini, gt, param, Indicator)
%%%%%%%%%%%%%%%%%%%%%%%%%% param
    V = length(X);
...
%         if norm(E2{1}) ~= 0
%             for v =1 :V
%                 alpha(v) = norm(E2{v}, inf);
%             end
%         end
        % updata Zr
        Z{V+1} = (mu*(H'*H)+ pho_mu*eye(Nsamp))\(H'*Y4 + mu*(H'*H) - mu*H'*Er- Y{V+1} +pho_mu*G{V+1} - 0.5*param.lambda3*Q');
...
% CONVERGENCE-------------------- 
        ...
        Norm4 = [Norm4 norm(H-H*Z{V+1}-Er,inf)];
    end
x=1:length(Norm1);
...
ylabel('Stop Critria');
legend('Xc-PH-E1','Xc-XcZ-E2','Z-G''H-HZr-Er')
